...
class Ajustes extends CI_Controller {
	...
	public function impuestoEdit(){
		...
		if ($this->input->post('impuesto_iva') == 1){
			$data['IVA'] = 1;
			unset($_SESSION[ 'iva' ]);
			$this->session->set_userdata ( 'iva' , 1);
		}else{
			$data['IVA'] = 0;
			unset($_SESSION[ 'iva' ]);
			$this->session->set_userdata ( 'iva' , 0);

		}
		
		if ($this->input->post('impuesto_tec') == 1){
			$data['TEC'] = 1;
			unset($_SESSION[ 'tec' ]);
			$this->session->set_userdata ( 'tec' , 1);
		}else{
			$data['TEC'] = 0;
			unset($_SESSION[ 'tec' ]);
			$this->session->set_userdata ( 'tec' , 0);
